...
describe('Listing Users', () => {
  it('returns 200 ok when there no user in database', async () => {
  ...
  it('return 6 users in page content when there are active 6 users and inactive 5 users in database', async () => {
    await addUsers(6, 5);
    const response = await getUsers();
  ...
  it('returns second page users and page indicator when pae is set as 1 in reuest parameter', async () => {
    await addUsers(11);
  ...
  it('returns firts page when page is set bellow zero as request parameter', async () => {
    await addUsers(11);
    const response = await getUsers().query({ page: -5 });
    expect(response.body.page).toBe(0);
  });

  it('it returns users and correspondig size indicator when size is set as 5 in request parameter', async () => {
    await addUsers(11);
    const response = await getUsers().query({ size: 5 });
    expect(response.body.content.length).toBe(5);
    expect(response.body.size).toBe(5);
  });
  it('it returns 10 users and corresponding size indicator whe size is set as 1000', async () => {
    await addUsers(11);
    const response = await getUsers().query({ size: 1000 });
    expect(response.body.content.length).toBe(10);
    expect(response.body.size).toBe(10);
  });
  it('it returns 10 users and corresponding size indicator whe size is set as 0', async () => {
    await addUsers(11);
...
describe('Get user', () => {
  ...
  `(`return $message for unknow user when language is set to $language`, async ({ language, message }) => {
